...
**Validation Rules:**
1. **Session Check:** User must be logged in as mahasiswa
2. **Ownership Check:** Booking must belong to current user (via NPM → id_mahasiswa)
3. **Status Check:** Only 'menunggu' can be cancelled
4. **Confirmation:** User must confirm cancellation

**Backend Logic:**
```php
// actions/cancel_booking.php
1. Get NPM from $_SESSION['user']
2. Query: SELECT id_mahasiswa FROM mahasiswa WHERE npm = ?
3. Verify booking ownership
4. Verify status = 'menunggu'
5. DELETE FROM antrian WHERE id_antrian = X
6. Return JSON {status: 'success'}
```
